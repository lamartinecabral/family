...
const getLocalidades = (() => {
  ...
  const fetcher = async () => {
    const csv = await fetch("./localidades_rows.csv").then((res) => res.text());
...
const getFrequencias = (() => {
  ...
  const fetcher = async () => {
    const csv = await fetch("./frequencias_rows.csv").then((res) => res.text());
    return schema.parse(parseCsv(csv));
  };
...
const getFrequenciasFromCompressed = (() => {
  ...
  const fetcher = async () => {
    const csv = await fetch("./frequencias_compressed.csv").then((res) =>
      res.text(),
    );
